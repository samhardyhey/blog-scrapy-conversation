...
class ConversationSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        article_links = response.xpath(
            "//a[contains(@href, 'theconversation.com') and contains(@href, '-')]/@href"
        ).getall()
        article_links = [
            link
        ...
        if not article_links:
            self.logger.warning(
                "No article links found. The website structure may have changed."
            )
            yield response.follow(
                "/au/business/articles",
                self.parse_articles_page,
                meta={"source_section": section},
            )
        else:
            for link in article_links[:10]:  # Limit to first 10 for testing
                yield response.follow(
                    link, self.parse_article, meta={"source_section": section}
                )
    ...
    def parse_article(self, response):
        ...
        author = (
            response.xpath('//*[contains(@class, "author-name")]/text()').get()
            or response.xpath('//*[contains(@class, "byline")]//text()').get()
            or response.xpath('//*[contains(@class, "author")]//text()').get()
            or response.xpath('//*[contains(@class, "contributor")]//text()').get()
            or "Unknown Author"
        )

        # Try multiple selectors for title
        title = (
            response.xpath('//h1[contains(@class, "entry-title")]//strong/text()').get()
            or response.xpath('//h1[contains(@class, "entry-title")]//text()').get()
            or response.xpath('//h1[contains(@class, "legacy")]//strong/text()').get()
            or response.xpath('//h1[contains(@class, "legacy")]//text()').get()
            or response.xpath('//meta[@property="og:title"]/@content').get()
            or response.xpath("//h1/text()").get()
            or response.xpath('//*[contains(@class, "headline")]//text()').get()
            or response.xpath('//*[contains(@class, "title")]//text()').get()
            or response.xpath('//*[contains(@class, "article-title")]//text()').get()
            or "Unknown Title"
        )
        ...
        published_text = (
            ...
            or response.xpath('//*[contains(@class, "published")]//text()').get()
            ...
            try:
                ...
                try:
                    ...
                    published = parser.parse(
                        " ".join(published_text.split(" ")[1:-1][::-1])
                    )
                except:
                    published = None
        ...
        # Try to get topics
        topics_elements = (
            response.xpath('//*[contains(@class, "topic-list-item")]').getall()
            or response.xpath('//*[contains(@class, "topic")]').getall()
            or response.xpath('//*[contains(@class, "tag")]').getall()
            or []
        )
        topics = (
            "|".join([BeautifulSoup(e, "lxml").text.strip() for e in topics_elements])
            if topics_elements
            else ""
        )
        ...
        self.logger.info(
            f"Extracted article: {title[:50]}... by {author} (content length: {len(article)})"
        )
