...
class ConversationSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        topics = response.xpath(
            '//*[contains(concat( " ", @class, " " ), concat( " ", "topic-list-item", " " ))]'
        ).getall()
        topics = "|".join([BeautifulSoup(e, "lxml").text.strip() for e in topics])

        article = response.xpath(
            '//*[contains(concat( " ", @class, " " ), concat( " ", "inline-promos", " " ))]//p | //h2'
        ).getall()
        article = [BeautifulSoup(e, "lxml").text.strip() for e in article]
        article = [
            p for p in article if all(skip not in p for skip in ["Read more", "Review"])
        ]
        article = "\n".join(article)

        yield {
            "author": response.xpath(
                '//*[contains(concat( " ", @class, " " ), concat( " ", "author-name", " " ))]/text()'
            )
            .get()
            .strip(),
